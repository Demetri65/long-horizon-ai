...
from src.schemas.edge import EdgeUnion  # not used here but kept for future
from src.services.policies import SYSTEM_POLICY, build_user_prompt  # keep your existing policy helpers

# Reusable adapters (Pydantic v2)
...
class Decomposer:
    ...
    async def decompose_goals(
        ...
        """
        # 1) start from your canonical prompt
        user_prompt = build_user_prompt(intent_text, max_nodes=max_goals)

        # 2) harden it to "goals-only"
        #    Keep it short—models respond better to precise rules near the end.
        user_prompt += (
            "\n\nSTRICT OUTPUT RULES:\n"
        ...
        result = await llm_client.generate_json(system=SYSTEM_POLICY, user=user_prompt)
        if not isinstance(result, dict) or "nodes" not in result:
            raise ValueError("LLM did not return an object with a 'nodes' array")
        ...
        if not goals_only:
            raise ValueError("No GOAL nodes returned by LLM")
