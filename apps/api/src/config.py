...
class ModelBase(BaseModel):
    ...
    model_config = ConfigDict(
        alias_generator=to_camel,          # snake_case -> camelCase on the wire
        populate_by_name=True,             # accept either snake or camel on input
        extra='forbid',                    # reject unknown fields
        str_strip_whitespace=True,
        validate_default=True,
        ser_json_inf_nan='null',            # strict JSON
    )
...
class Settings(BaseSettings):
    ...
    OPENAI_MODEL: str = Field(
        default="gpt-5-mini",  # or your preferred default
