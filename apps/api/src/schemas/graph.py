...
from .node import NodeUnion
from .edge import EdgeUnion, ContributesToEdge
...
class Graph(ModelBase):
    ...
    def _iter_metrics(node) -> Iterable:
        ...
        if meas is None:
            return []
        if isinstance(meas, list):           # current schema: list[Metric]
            return meas
        return getattr(meas, "metrics", []) or []  # legacy: object with .metrics
    ...
    def upsert_node(self, node: NodeUnion) -> "Graph":
        ...
        # 1) Replace in-place if it already exists anywhere in the tree
        ...
        if parent_id is not None:
            ...
            if not isinstance(host_children, list):
                # materialize children list if missing (Pydantic or dict node)
                try:
                    setattr(host, "nodes", [])
